...
Public Class FormDataDosenAll
    ...
    Private Sub GET_DELETE_RECORD(ByVal NIP_NIDN As String, ByVal aFileName As String)
            ...
            Using cmd As SqlClient.SqlCommand = New SqlClient.SqlCommand(query, conn)
                Try
                    With cmd
                        ...
                        .CommandText = query
                        .Connection.Open()
                        .Parameters.Add("@NIP_NIDN", SqlDbType.VarChar).Value = NIP_NIDN
                        .ExecuteNonQuery()
                        .Connection.Close()
        ...
        Try
            For Each row As DataGridViewRow In Me.DataGridViewX1.Rows
                If row.Cells(0).Value = True Then
                    ...
                    Me.GET_DELETE_RECORD(row.Cells(1).Value, row.Cells(4).Value)
                End If
            Next
    ...
    Private Sub BackgroundWorkerSearch_DoWork(sender As Object, e As System.ComponentModel.DoWorkEventArgs) Handles BackgroundWorkerSearch.DoWork
        Me.TampilDataFormDialog()
        Dim strQuery As String = "select * from T_DOSEN WHERE NIP_NIDN LIKE @p1 Or NAMA_DOSEN LIKE @p2"
