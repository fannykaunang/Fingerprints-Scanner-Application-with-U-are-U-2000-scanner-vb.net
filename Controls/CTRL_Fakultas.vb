...
#Region "SetCultureToIndonesia"
    Private Sub SetCultureToIndonesia()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(CTRL_FAKULTAS))
...
#End Region
    ...
    Private Sub NOMOR_OTOMATIS()
        Using connect As New SqlConnection(Me.STRCONN_STRING.ToString)
            Dim str As String = "SELECT (ABS(CHECKSUM(NEWID())) % 100001) + ((ABS(CHECKSUM(NEWID())) % 100001) * 0.00001) [KODE_FAKULTAS]"
    ...
    Private Function VALIDATE_INPUT() As Boolean
        If Me.TXTID.Text = String.Empty _
             Or Me.TXTFAKULTAS.Text = String.Empty _
            Then
...
#Region "SQL Query Manipulation"
    Private Sub GET_INSERT_DATA()
        Try
            Dim STR_SQL As String = "INSERT INTO T_FAKULTAS "
            STR_SQL &= "VALUES(@KODE_FAKULTAS, @FAKULTAS)"
            Using CONNECTION As New SqlConnection(Me.STRCONN_STRING.ToString)
                Using COMMAND_PARAM As SqlClient.SqlCommand = New SqlClient.SqlCommand(STR_SQL, CONNECTION)
                    ...
                    With COMMAND_PARAM
                        ...
                        .CommandType = CommandType.Text
                        .CommandText = STR_SQL
                        .Parameters.Add("@KODE_FAKULTAS", SqlDbType.VarChar).Value = Me.TXTID.Text
                        .Parameters.Add("@FAKULTAS", SqlDbType.VarChar).Value = Me.TXTFAKULTAS.Text
                        .Connection.Open()
                        .ExecuteNonQuery()
                        .Connection.Close()
                    End With

                    MessageBoxEx.Show("DATA FAKULTAS DENGAN NAMA : " & Me.TXTFAKULTAS.Text & vbNewLine & "BERHASIL DISIMPAN.", My.Application.Info.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Information)
                    Me.IS_EDITING = False
    ...
    Private Sub GET_UPDATE_DATA()
        Dim STR_SQL As String = "UPDATE T_FAKULTAS SET "
        STR_SQL &= "FAKULTAS=@FAKULTAS, KODE_FAKULTAS=@KODE_FAKULTAS WHERE KODE_FAKULTAS=@KODE_FAKULTAS"
        Using CONNECTION As New SqlConnection(Me.STRCONN_STRING.ToString)
            Using COMMAND_PARAM As SqlClient.SqlCommand = CONNECTION.CreateCommand
                Try
                    ...
                    MessageBoxEx.Show("DATA FAKULTAS DENGAN NAMA : " & Me.TXTFAKULTAS.Text & vbNewLine & "BERHASIL DIUBAH.", My.Application.Info.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Information)
                    Me.PictureBox1.Image = My.Resources.accept_icon
    ...
    Private Sub GET_DELETE_RECORD(ByVal Kode_D_W As String)
        Dim query As String = "DELETE FROM T_FAKULTAS WHERE KODE_FAKULTAS = @KODE_FAKULTAS"
            ...
            Using cmd As SqlClient.SqlCommand = New SqlClient.SqlCommand(query, conn)
                Try
                    With cmd
                        ...
                        .CommandText = query
                        .Connection.Open()
                        .Parameters.Add("@KODE_FAKULTAS", SqlDbType.VarChar).Value = Kode_D_W
                        .ExecuteNonQuery()
                        .Connection.Close()
                    End With
...
#End Region
    ...
    Private Sub BTUBAH_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BTUBAH.Click
        If Me.DataGridViewX1.CurrentRow.Cells("KODE_FAKULTAS").Selected = False AndAlso Me.DataGridViewX1.CurrentRow.Cells("FAKULTAS").Selected = False Then
        ...
        Try
            Dim SQL As String = "SELECT * FROM T_FAKULTAS WHERE KODE_FAKULTAS = @KODE_FAKULTAS"
            Using conn As New SqlConnection(Me.STRCONN_STRING.ToString)
                Using cmd As SqlClient.SqlCommand = New SqlClient.SqlCommand(SQL, conn)
                    conn.Open()
                    cmd.CommandType = CommandType.Text
                    cmd.Parameters.Add("@KODE_FAKULTAS", SqlDbType.VarChar).Value = Me.DataGridViewX1.CurrentRow.Cells("KODE_FAKULTAS").Value
                    Using r As SqlClient.SqlDataReader = cmd.ExecuteReader()
                        If r.HasRows() Then
                            If r.Read() Then
                                Dim p As New Point(Me.PanelEx1.Width, 0)
                                a.Start(DataGridViewX1, p, MetroFramework.Animation.TransitionType.EaseInCubic, 15)
                                Me.TXTID.Text = r("KODE_FAKULTAS").ToString
                                Me.TXTFAKULTAS.Text = r("FAKULTAS").ToString
                                Me.jenisOperasi = operasi.update
                            End If
                        Else
                            Me.TXTID.Text = String.Empty
                            Me.TXTFAKULTAS.Text = String.Empty
                        End If
                        conn.Close()
                    End Using
                End Using
...
#Region "ASYNCHRONOUS"
    ...
    Private Sub BackgroundWorker_DoWork(ByVal sender As System.Object, ByVal e As System.ComponentModel.DoWorkEventArgs) Handles BackgroundWorker.DoWork
        ...
        Dim strQuery As String = "select * from T_FAKULTAS ORDER BY KODE_FAKULTAS"
        Try
            Using conn As New SqlConnection(Me.STRCONN_STRING.ToString)
...
#End Region
    ...
    Private Sub BackgroundWorkerSearch_DoWork(ByVal sender As System.Object, ByVal e As System.ComponentModel.DoWorkEventArgs) Handles BackgroundWorkerSearch.DoWork
        Me.TampilDataFormDialog()
        Dim strQuery As String = "select * from T_FAKULTAS WHERE KODE_FAKULTAS LIKE @p1 Or FAKULTAS LIKE @p2"
        strQuery &= " ORDER BY KODE_FAKULTAS"
